...
static kz_thread_id_t thread_run(kz_func_t func, char * name,
  ...
  *(--sp) = (unit32)thread_end;
  *(--sp) = (unit32)thread_init;
  // ER6 - ER1
  *(--sp) = 0;
  *(--sp) = 0;
  *(--sp) = 0;
  ...
  *(--sp) = 0;
  ...
  // ER0(pass to startup of thread)
  *(--sp) = (uint32)thp;

  // set thread context
  thp->context.sp = (uint32)sp;

  // back to new thread to ready queue
  putcurrent();
...
static int thread_exit(void) {
  // ! omit the processes of releasing stack
...
static int setintr(softvec_type_t type, kz_handler_t handler) {
  ...
  /* 
    register entry function of OS-interruption 
    to software interruption vectors
  */
...
void kz_start(kz_func_t func, char * name, int stacksize,
  ...
  memset(threads, 0, sizeof(handlers));

  setintr(SOFTVEC_TYPE_SYSCALL, syscall_intr);
  setintr(SOFTVEC_TYPE_SOFTERRm softerr_intr);
